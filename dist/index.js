...
const STORYBOOK_URL = process.env.STORYBOOK_URL || "https://pds.phenom.com/angular";
// ─── Helpers ───────────────────────────────────────────────────────────────
...
function extractInputs(componentTsPath) {
...
function extractOutputs(componentTsPath) {
    const project = new Project({ skipAddingFilesFromTsConfig: true });
    const sourceFile = project.addSourceFileAtPath(componentTsPath);
    const results = [];
    for (const cls of sourceFile.getClasses()) {
        for (const prop of cls.getProperties()) {
            ...
            const eventType = typeArgs.length > 0 ? typeArgs[0].getText(prop) : "unknown";
            const jsDocs = prop.getJsDocs();
            const description = jsDocs.length > 0
                ? jsDocs
                    .map((d) => {
...
async function fetchStorybookIndex() {
    ...
    if (!res.ok)
        throw new Error(`Storybook not reachable at ${STORYBOOK_URL}`);
    return res.json();
}
// ─── MCP Server ────────────────────────────────────────────────────────────
const server = new Server({ name: "phenom-angular-mcp", version: "1.0.0" }, { capabilities: { tools: {} } });
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        {
            name: "list_components",
            description: "List all Angular components in the Phenom Design System repo. Returns component name, selector, file path, and whether a story exists.",
            inputSchema: {
                type: "object",
                properties: {
                    filter: {
        ...
        },
        {
            ...
            description: "Get the full source code of an Angular component: TypeScript class, HTML template, and SCSS styles.",
            inputSchema: {
                type: "object",
                properties: {
                    componentName: {
        ...
        },
        {
            ...
            description: "Get all @Input() properties of an Angular component with types, aliases, required flag, default values, and JSDoc descriptions.",
            inputSchema: {
                type: "object",
                properties: {
                    componentName: {
        ...
        },
        {
            ...
            description: "Get all @Output() EventEmitter properties of an Angular component with event types.",
            inputSchema: {
                type: "object",
                properties: {
                    componentName: {
        ...
        {
            name: "get_story_code",
            description: "Get the Storybook story file (.stories.ts) for an Angular component.",
            inputSchema: {
                type: "object",
                properties: {
                    componentName: {
        ...
        },
        {
            ...
            description: "Search components in the Phenom DS by name, selector, or keyword.",
            inputSchema: {
                type: "object",
                properties: {
                    query: {
        ...
        },
        {
            ...
            description: "Fetch the live Storybook index (requires Storybook to be running). Returns all stories with their IDs, names, and component groupings.",
            inputSchema: {
                type: "object",
                properties: {},
            },
        ...
        switch (name) {
            ...
            case "list_components": {
                ...
                const filtered = filter
                    ? components.filter((c) => c.name.toLowerCase().includes(filter) ||
            ...
            case "get_component_source": {
                const query = (args?.componentName).toLowerCase();
                const components = await scanComponents();
                const match = components.find((c) => c.name.toLowerCase().includes(query) ||
            ...
            case "search_components": {
                ...
                const components = await scanComponents();
                const results = components.filter((c) => c.name.toLowerCase().includes(query) ||
                    c.selector?.toLowerCase().includes(query) ||
                    c.filePath.toLowerCase().includes(query));
                return {
                    content: [
